...
window.PYTHONLAB_SPRAK.sv = {
  ...
  "nastan": "Nästan klart",
  "forbereder": "Förbereder labbet",
  "verktyg": "Kopplar in labbets verktyg",
  "igen": "Försöker igen",
  "byter": "Byter källa",
  "klart": "Klart",
  "redo": "Python är redo",
  ...
  },
  {
    ...
    forklaring: `
      ...
      <ul>
        <li><code>data.download("djur.csv")</code> läser filen ur labbet och skickar innehållet till webbläsaren, som gör en vanlig nedladdning. Samma sak som att klicka på en nedladdningslänk.</li>
        <li><code>data.open_file()</code> öppnar en filväljare. Den fil <em>du</em> pekar ut kopieras in i labbets filsystem, och kan sedan läsas med vanlig <code>open()</code>. Det är ditt aktiva val som ger tillåtelsen — programmet kan inte välja åt dig.</li>
      </ul>
      <p>Genvägar finns: <code>data.open_csv()</code> ger direkt en lista med dictionaries, och <code>data.open_json()</code> ger tillbaka det som sparades. <code>data.list_files()</code> visar vad som ligger i labbet just nu.</p>
      ...
      <div class="uppgift"><strong>Uppgift:</strong> bygg en dagbok i sqlite3. En funktion lägger till datum och text, en annan hämtar alla poster, och en tredje söker efter ett ord med <code>WHERE text LIKE ?</code> och mönstret <code>"%sökord%"</code>. Exportera sedan hela dagboken till CSV och ladda ner den — och nästa gång du öppnar labbet, läs tillbaka den med <code>data.open_csv()</code> och fyll databasen igen. Då har du en dagbok som faktiskt överlever.</div>`
  },
  {
  ...
  },
  {
    ...
    forklaring: `
      ...
      <p>Pythonlabbet kör Python inuti en webbsida, och på samma sida finns <strong>three.js</strong> — ett JavaScript-bibliotek för 3D-grafik. Modulen <code>scene</code> är en bro mellan de två: du skriver Python, och 3D-motorn ritar.</p>
  ...
  },
  {
    ...
    forklaring: `
      ...
      <p>Modulen <code>data</code> finns bara i Pythonlabbet. Den har tre funktioner: <code>fetch_json(url)</code>, <code>fetch_text(url)</code> och <code>fetch_csv(url)</code>. På en riktig dator använder man biblioteket <code>requests</code> i stället, men principen är densamma.</p>
      <h3>JSON</h3>
